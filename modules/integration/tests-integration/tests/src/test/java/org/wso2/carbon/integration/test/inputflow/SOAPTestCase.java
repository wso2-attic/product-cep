...
public class SOAPTestCase extends CEPIntegrationTest {
    ...
    public void SOAPXMLTestScenario() throws Exception {
        ...
        String streamDefinitionAsString = getJSONArtifactConfiguration("inputflows/sample0014",
                "org.wso2.event.sensor.stream_1.0.0.json");
        eventStreamManagerAdminServiceClient.addEventStreamAsString(streamDefinitionAsString);
        Assert.assertEquals(eventStreamManagerAdminServiceClient.getEventStreamCount(), startESCount + 1);

        //Add SOAP JSON EventReceiver without mapping
        String eventReceiverConfig = getXMLArtifactConfiguration("inputflows/sample0014", "soapReceiver.xml");
        ...
        String eventPublisherConfig2 = getXMLArtifactConfiguration("inputflows/sample0014", "wso2EventPublisher.xml");
        eventPublisherAdminServiceClient.addEventPublisherConfiguration(eventPublisherConfig2);
        Assert.assertEquals(eventPublisherAdminServiceClient.getActiveEventPublisherCount(), startEPCount + 1);

        // The data-bridge receiver
        TestAgentServer agentServer = new TestAgentServer("inputflows/sample0014", 7661, true);
        Thread agentServerThread = new Thread(agentServer);
        agentServerThread.start();
        // Let the server start
        Thread.sleep(10000);

        SOAPEventPublisherClient.publish("http://localhost:9763/services/soapReceiver/receive",
                "inputflows/sample0014", "soapReceiver.txt");
